...
class Temperal_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.transformer_encoder(self.x_dense) + self.x_dense  #[N, H, D]
        self.x_endoced=self.mlp(output) + output#[N, D]

        return self.x_endoced
...
class Global_interaction(nn.Module):
    def __init__(self,args):
        ...
        # Attention
        self.classMLP =  MLP(2, self.hidden_size)
        self.WAr = MLP(self.hidden_size*3, 1)
        self.weight = MLP(self.hidden_size)

        
    def forward(self, corr_index, nei_index, nei_num, hidden_state, cn,agent_class):
        '''
        ...
        nei_agent_class_1 = agent_class.repeat(self.N).contiguous().view(-1,1)
        nei_agent_class_2 = nei_agent_class_1.view(self.N,self.N).permute(0,1).contiguous().view(-1,1)
        nei_agent_class = torch.cat((nei_agent_class_1,nei_agent_class_2),dim=1)
        nei_agent_class = self.classMLP(nei_agent_class)
        ...
        # tmp = torch.cat((r_t, hi_t[nei_index_t > 0],nei_inputs[nei_index_t > 0],nei_agent_class[nei_index_t > 0]), 1) #[N*N, 3*D]
        # Motion Gate
